...
mra_wendland_2d_pred <- function(
    ...
    for (m in 1:M) {
        ...
        D_pred <- rdist(locs_pred, locs_grid[[m]])
        if (use_spam) {
            ## use the spam sparse matrix package
            W_pred[[m]] <- spam(c(wendland_basis(D_pred, radius[m])), nrow = nrow(D_pred), ncol = ncol(D_pred))
        } else {
            ## use the Matrix sparse matrix package
            W_pred[[m]] <- Matrix(wendland_basis(D_pred, radius[m]), sparse = TRUE)
        }
    }
    return(
        list(
