...
predict_pg_stlm_overdispersed <- function(
    out,
    X,
    X_pred,
    locs,
    ...
    for (k in 1:n_samples) {
        if (shared_covariance_params) {
            ...
            if (corr_fun == "matern") {
                Sigma <- tau2[k] * correlation_function(D_obs, theta[k, ], corr_fun = corr_fun) + sigma2[k] * I
                if (!posterior_mean_only) {
                    ## only calculate if we are estimating the full posterior distribution
                    Sigma_unobs <- tau2[k] * correlation_function(D_pred, theta[k, ], corr_fun = corr_fun) + sigma2[k] * I
                }
            ...
            for (j in 1:(J - 1)) {
                ...
                Sigma_unobs_obs <- NULL
                if (corr_fun == "matern") {
                    Sigma <- tau2[k, j] * correlation_function(D_obs, theta[k, j, ], corr_fun = corr_fun) + sigma2[k, j] * I
                    if (!posterior_mean_only) {
                        ## only calculate if we are estimating the full posterior distribution
                        Sigma_unobs <- tau2[k, j] * correlation_function(D_pred, theta[k, j, ], corr_fun = corr_fun) + sigma2[k, j] * I
                    }
