...
df <- 4
Xbs <- bs(X, df)
beta <- matrix(rnorm((d-1) * df), df, (d-1))
## make the intercepts smaller to reduce stochastic ordering effect
beta[1, ] <- beta[1, ] - seq(from = 4, to = 0, length.out = d - 1)
eta <- Xbs %*% beta
pi <- eta_to_pi(eta)
...
Why is the parallelization not helping the runtime? what about using the foreach/future type parallelization used before?
...
inits <- default_inits_pgLM(Y, X, priors)
...
if (file.exists(here::here("results", "timings-pgLM.RData"))) {
  ...
  time_8 <- system.time(
    out <- pgLM(Y, as.matrix(Xbs), params, priors, n_cores = 8L)
  )
  time_6 <- system.time(
    out <- pgLM(Y, as.matrix(Xbs), params, priors, n_cores = 6L)
  )
  time_4 <- system.time(
    out <- pgLM(Y, as.matrix(Xbs), params, priors, n_cores = 4L)
  )
  time_2 <- system.time(
    out <- pgLM(Y, as.matrix(Xbs), params, priors, n_cores = 2L)
  )
  time_1 <- system.time(
    out <- pgLM(Y, as.matrix(Xbs), params, priors, n_cores = 1L)
  )
...
#     "knot" = 1:dim(beta_post)[2],
#     "species" = 1:dim(beta_post)[3]
# )
# dimnames(beta) <- list(
#     "knot" = 1:dim(beta)[1],
#     "species" = 1:dim(beta)[2]
# )
# dat_truth <- as.data.frame.table(beta, responseName = "beta_truth")
# dat_plot <- as.data.frame.table(beta_post, responseName = "beta")
...
dat_plot <- data.frame(
    ...
    type = rep(c("estimate", "truth"), each = (d-1) * ncol(Xbs)),
    species = factor(rep(1:(d-1), each = ncol(Xbs))),
    knots = 1:ncol(Xbs)
)
