...
if (file.exists(here::here("results", "pgSPLM-shared.RData"))) {
  ...
  start <- Sys.time()
  out <- pgSPLM(Y, as.matrix(X), as.matrix(locs), params, priors, n_cores = 6L, shared_covariance_params = TRUE)
  stop <- Sys.time()
  runtime <- stop - start
...
if (file.exists(here::here("results", "pgSPLM-sample-shared.RData"))) {
  ...
  ## Q: faster to parallelize using openMP or foreach?
  start <- Sys.time()
  out <- pgSPLM(Y_s, as.matrix(X_s), as.matrix(locs_s), params, priors, n_cores = 6L, shared_covariance_params = TRUE)
  stop <- Sys.time()
  runtime <- stop - start
...
params$n_thin <- 1
priors <- default_priors_pgSPLM(Y, X)
inits  <- default_inits_pgSPLM(Y, X, priors, shared_covariance_params = FALSE)
...
if (file.exists(here::here("results", "pgSPLM.RData"))) {
  ...
  ## Q: faster to parallelize using openMP or foreach?
  start <- Sys.time()
  out <- pgSPLM(Y, as.matrix(X), as.matrix(locs), params, priors, n_cores = 6L, shared_covariance_params = FALSE, verbose = FALSE)
  stop <- Sys.time()
  runtime <- stop - start
...
params$n_thin <- 1
priors <- default_priors_pgSPLM(Y_s, X_s)
inits <- default_inits_pgSPLM(Y_s, X_s, priors, shared_covariance_params = FALSE)
...
if (file.exists(here::here("results", "pgSPLM-sample.RData"))) {
  ...
  ## Q: faster to parallelize using openMP or foreach?
  start <- Sys.time()
  out <- pgSPLM(Y_s, as.matrix(X_s), as.matrix(locs_s), params, priors, n_cores = 6L, shared_covariance_params = FALSE)
  stop <- Sys.time()
  runtime <- stop - start
