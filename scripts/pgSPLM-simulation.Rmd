...
inits <- default_inits_pgSPLM(Y, X, priors)
...
if (file.exists(here::here("results", "pgSPLM.RData"))) {
  ...
  start <- Sys.time()
  out <- pgSPLM(Y, as.matrix(X), as.matrix(locs), params, priors, n_cores = 6L)
  stop <- Sys.time()
  runtime <- stop - start
...
inits <- default_inits_pgSPLM(Y_s, X_s, priors)
...
if (file.exists(here::here("results", "pgSPLM-sample.RData"))) {
  ...
  ## Q: faster to parallelize using openMP or foreach?
  start <- Sys.time()
  out <- pgSPLM(Y_s, as.matrix(X_s), as.matrix(locs_s), params, priors, n_cores = 6L)
  stop <- Sys.time()
  runtime <- stop - start
...
if (file.exists(here::here("results", "pgSPLM-sample-preds.RData"))) {
  ...
  preds <- predict_pgSPLM(out, X_s, X_oos, locs_s, locs_oos)
