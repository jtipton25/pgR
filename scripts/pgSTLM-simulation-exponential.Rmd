---
title: "pgSTLM Simulation test - exponential covariance function"
bibliography: pg.bib
output:
...
# Something is happening where the sampler for theta is getting stuck


We are testing the spatial Polya-gamma linear model `pgSTLM()`
...
##
missing_idx <- matrix(FALSE, n_sites, n_time)
N_missing <- ceiling(n_sites * n_time * 0.25) ## 25% of observations missing
missing_idx[sample(1:(n_sites * n_time), N_missing)] <- TRUE
Y_missing <- Y_s
# mean(missing_idx)
...
dat_pi_s     <- as.data.frame.table(pi_s, responseName = "pi")
...
dat_plot <- merge(dat_pi_s, dat_locs_s)
...
if (file.exists(here::here("results", "pgSTLM-exponential-shared.RData"))) {
  ...
  start <- Sys.time()
  out <- pgSTLM(Y_missing, as.matrix(X_s), as.matrix(locs_s), params, priors, corr_fun = "exponential",  n_cores = 6L, shared_covariance_params = TRUE, 
                sample_beta = 
                  TRUE,
                # FALSE,
                sample_tau2 = 
...
## Predicting at unobserved locations during observed time periods

Notation: $s$ represents the observed (sampled) locations and $oos$ represents the unobserved (out of sample) locations.
...
```{r, message = FALSE}
## predict the unobserved locations
if (file.exists(here::here("results", "pgSTLM-exponential-sample-preds-shared.RData"))) {
  load(here::here("results", "pgSTLM-exponential-sample-preds-shared.RData"))
} else {
  ## currently this is somewhat slow but is embarassingly parallel
  preds <- predict_pgSTLM(out, X_s, X_oos, locs_s, locs_oos, corr_fun = "exponential", shared_covariance_params = TRUE)
  save(preds, file = here::here("results", "pgSTLM-exponential-sample-preds-shared.RData"))
}
```
...
dat_plot %>% 
  ...
  facet_grid(time ~ species) +
  geom_abline(intercept = 0, slope = 1, col = "red") +
  ggtitle("Kriging estimate vs. simulated eta")
```


```{r}
## plot predicited pi
pi_pred_mean <- array(0, dim = c(N-n_sites, J, n_time))
for (tt in 1:n_time) {
...
dat_plot %>% 
  ...
  facet_grid(time ~ species) +
  geom_abline(intercept = 0, slope = 1, col = "red") +
  ggtitle("Kriging estimate vs. simulated pi")
```
