---
title: "pg_splm_mra Simulation test"
bibliography: pg.bib
output:
...
alpha <- matrix(0, sum(MRA$n_dims), J-1)
for (j in 1:(J-1)) {
  alpha[, j] <- rmvnorm.prec.const(n = 1, mu = rep(0, sum(MRA$n_dims)), Q = Q_alpha_tau2[[j]])
}
W_alpha <- W %*% alpha
...
eta <- X %*% beta + W_alpha + sapply(1:(J-1), function(j) rnorm(N, 0, sqrt(sigma2[j])))
pi <- eta_to_pi(eta)
...
if (file.exists(here::here("results", "pg_splm-mra.RData"))) {
  load(here::here("results", "pg_splm-mra.RData"))
} else {
  ...
  start <- Sys.time()
  out <- pg_splm_mra(Y, as.matrix(X), as.matrix(locs), params, priors, M = 3, n_coarse_grid = 6, n_cores = 1L)
  stop <- Sys.time()
  runtime <- stop - start
  
  save(out, runtime, file = here::here("results", "pg_splm-mra.RData"))
}
```
...
dimnames(beta) <- list(
  parameter = paste0("beta[", 1:dim(betas)[2], "]"),
  species   = paste0("species[", 1:dim(betas)[3], "]")
)
...
as.data.frame.table(betas, responseName = "value") %>%
  ...
  geom_point(data = as.data.frame.table(betas, responseName = "value"), color = "red")  
...
## typically, tau2 is not identifiable
plot(apply(out$tau2, c(2, 3), mean), tau2)
abline(0, 1, col = "red")
```
...
if (file.exists(here::here("results", "pg_splm-mra-sample.RData"))) {
  load(here::here("results", "pg_splm-mra-sample.RData"))
} else {
  ## need to parallelize the polya-gamma random variables
  ## can I do this efficiently using openMP?
  ## Q: faster to parallelize using openMP or foreach?
  start <- Sys.time()
  out <- pg_splm_mra(Y_s, as.matrix(X_s), as.matrix(locs_s), params, priors, M = 3, n_coarse_grid = 6, n_cores = 1L)
  stop <- Sys.time()
  runtime <- stop - start
  
  save(out, runtime, file = here::here("results", "pg_splm-mra-sample.RData"))
}
```
...
```{r, message = FALSE}
if (file.exists(here::here("results", "pg-splm-mra-sample-preds.RData"))) {
  load(here::here("results", "pg-splm-mra-sample-preds.RData"))
} else {
  ## now this is super fast
  preds <- predict_pg_splm_mra(out, X_s, X_oos, locs_s, locs_oos)
  save(preds, file = here::here("results", "pg-splm-mra-sample-preds.RData"))
}
```
