...
Q_alpha <-  make_Q_alpha_2d(sqrt(n_dims), rep(0.999, length(n_dims)))
...
for (j in 1:(J-1)) {
  alpha[, j] <- rmvnorm.prec.const(n = 1, mu = rep(0, sum(n_dims)), Q = Q_alpha_tau2[[j]])
}
W_alpha <- W %*% alpha
...
if (file.exists(here::here("results", "pg_splm-mra.RData"))) {
  ...
  start <- Sys.time()
  out <- pg_splm_mra(Y, as.matrix(X), as.matrix(locs), params, priors, M = 3, n_coarse_grid = 6, n_cores = 6L)
  stop <- Sys.time()
  runtime <- stop - start
...
if (file.exists(here::here("results", "pg_splm-mra-sample.RData"))) {
  ...
  ## Q: faster to parallelize using openMP or foreach?
  start <- Sys.time()
  out <- pg_splm_mra(Y_s, as.matrix(X_s), as.matrix(locs_s), params, priors, M = 3, n_coarse_grid = 6, n_cores = 6L)
  stop <- Sys.time()
  runtime <- stop - start
