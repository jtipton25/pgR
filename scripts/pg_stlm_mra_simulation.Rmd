...
for (j in 1:(J-1)) {
  alpha[, j, 1] <- rmvnorm.prec.const(n = 1, mu = rep(0, sum(n_dims)), Q = Q_alpha_tau2[[j]])
}
for (tt in 2:n_time) {
  for (j in 1:(J-1)) {
...
if (file.exists(here::here("results", "pg_stlm-mra.RData"))) {
  ...
  start <- Sys.time()
  out <- pg_stlm_mra(Y, as.matrix(X), as.matrix(locs), params, priors, M = 3, n_coarse_grid = 6, n_cores = 6L)
  stop <- Sys.time()
  runtime <- stop - start
...
if (file.exists(here::here("results", "pg_stlm-mra-sample.RData"))) {
  ...
  start <- Sys.time()
  out <- pg_stlm_mra(Y_s, as.matrix(X_s), as.matrix(locs_s), params, priors, M = 3, n_coarse_grid = 6, n_cores = 6L, verbose = TRUE)
  stop <- Sys.time()
  runtime <- stop - start
