...
fun_idx <- function(cores = 1L, threshold = 1e6L) {
    tmp <- matrix(0, N, d-1)
    tmp[nonzero_idx] <- pgdraw(Mi[nonzero_idx], eta[nonzero_idx], cores = cores, threshold = threshold)
    tmp
}
...
if (file.exists(here::here("results", "timings-pgdraw.RData"))) {
    ...
    bm <- microbenchmark::microbenchmark(
        fun_sequential(threshold = 1e6L),
        fun_idx(cores = 1L, threshold = 1e6L),
        fun_idx(cores = 2L, threshold = 1e6L),
        fun_idx(cores = 4L, threshold = 1e6L),
        fun_idx(cores = 6L, threshold = 1e6L),
        fun_idx(cores = 8L, threshold = 1e6L),
        # speed comparison to the approximate sampler using CLT
        fun_sequential(threshold = 170L),
        fun_idx(cores = 1L, threshold = 170L),
        fun_idx(cores = 2L, threshold = 170L),
        fun_idx(cores = 4L, threshold = 170L),
        fun_idx(cores = 6L, threshold = 170L),
        fun_idx(cores = 8L, threshold = 170L)
)
